...
class DelCall(StatesGroup):
    general_date = State()

@dp.message_handler(commands=['del_call'], state=None)
...
@dp.message_handler(state=DelCall.general_date)
async def general_date_catch(message: Message, state: FSMContext):
    try:
        ...
        db.remove_call(up_data)
        await message.answer(text=f'данные звонков за {data_obj} удалены')
