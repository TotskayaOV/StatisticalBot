...
class DelEvol(StatesGroup):
    general_date = State()

@dp.message_handler(commands=['del_evol'], state=None)
...
@dp.message_handler(state=DelEvol.general_date)
async def general_date_catch(message: Message, state: FSMContext):
    try:
        data_obj = message.text
        up_data = datetime.strptime(data_obj, '%Y-%m-%d').date()
        db.remove_evolutions(up_data)
        await message.answer(text=f'данные оценок за {data_obj} удалены')
