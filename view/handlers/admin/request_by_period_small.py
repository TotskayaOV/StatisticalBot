...
from controller import read_wb_period_only_total


class PeriodSmallDate(StatesGroup):
    begin_date = State()
    str_begin = State()
    end_date = State()
    str_end = State()

@dp.message_handler(commands=['pd_date'], state=None)
async def add_user(message: Message, admin: bool):
    if admin:
        ...
        await PeriodSmallDate.next()
    else:
        await message.answer('У вас нет доступа к этой функции')

@dp.message_handler(state=PeriodSmallDate.begin_date)
async def general_date_catch(message: Message, state: FSMContext):
    try:
        ...
        await PeriodSmallDate.end_date.set()
    except:
        await message.answer(text='Ошибка ввода даты', reply_markup=kb_cancel_fsm)

@dp.message_handler(state=PeriodSmallDate.end_date)
async def general_date_catch(message: Message, state: FSMContext):
    try:
        data_obj = message.text
        up_data = datetime.strptime(data_obj, '%Y-%m-%d')
        ...
        await PeriodSmallDate.str_end.set()
    except:
        await message.answer(text='Ошибка ввода даты', reply_markup=kb_cancel_fsm)

@dp.message_handler(state=PeriodSmallDate.str_end)
async def general_date_catch(message: Message, state: FSMContext):
    if message.text == 'Да':
        try:
            data = await state.get_data()
            result = read_wb_period_only_total({'name_beg_date': data.get('str_begin'),
                                                'name_end_date': data.get('str_end'),
                                                'beg_date': data.get('begin_date'),
